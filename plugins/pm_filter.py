...
async def next_page(bot, query):
    ...
    if settings['button']:
        ...
        btn.insert(0, [
            InlineKeyboardButton("Sᴛᴀʀᴛ Bᴏᴛ", url=f"https://telegram.me/yoonseri2bot"),
...
async def cb_handler(client: Client, query: CallbackQuery):
    ...
    if query.data.startswith("file"):
                        ...
                        await client.send_cached_media(
                            ...
                            reply_markup=InlineKeyboardMarkup(
                                ...
                                [
                                    ...
                                    InlineKeyboardButton(" ᴍᴀɪɴ ᴄʜᴀɴɴᴇʟ ", url="t.me/FT_Channels")
                                ]
                                ]
                            )
                        )
        ...
        buttons = [[
            InlineKeyboardButton('ᴀɴʏ ǫᴜᴇʀʏ ✉️', url='t.me/FT_Adminbot')
        ], [
    ...
    [
        ...
        await query.message.edit_text(
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("Cᴏɴᴛᴀᴄᴛ", url="t.me/FTAdminbot")
                  ]]
            await client.edit_message_media(
                query.message.chat.id, 
                query.message.id, 
            ...
            btn = [[
                    ...
                    InlineKeyboardButton("↼ ʙᴏᴛ ᴄʜᴀɴɴᴇʟ ⇀", url="t.me/FT_Channels")
                ],[
...
async def auto_filter(client, msg, spoll=False):
    ...
    if not spoll:
        ...
        text = message.text or ""   # fallback to empty string if None
        if re.findall(r"((^\/|^,|^!|^\.|^[\U0001F600-\U000E007F]).*)", text):
            return
        if len(message.text) < 100:
            search = message.text
            btn = [[
                    InlineKeyboardButton(f"Searching  🔍  for {search}", url="t.me/FT_Channels")
                    ]]
    ...
    settings = await get_settings(message.chat.id) # ensure settings are fetched
    pre = 'filep' if settings['file_secure'] else 'file'
    if settings["button"]:
        btn = [
            [
        ...
        btn = [
            [
                InlineKeyboardButton(
                    text=f"{' '.join(filter(lambda x: not x.startswith('[') and not x.startswith('@') and not x.startswith('www.') and not x.startswith('tg') and not x.startswith('telegram'), file.file_name.split()))}", callback_data=f'{pre}#{file.file_id}',
                ),
                InlineKeyboardButton(
    ...
    if not g_s:
        ...
        button = [[
                   InlineKeyboardButton("🔍 Click To Check Spelling ", url=f"https://www.google.com/search?q={reqst_gle}")
                    ...
                    InlineKeyboardButton("📌 Join For New Movies", url=f"https://t.me/FT_Channels")
                ]]
        if NO_RESULTS_MSG:
